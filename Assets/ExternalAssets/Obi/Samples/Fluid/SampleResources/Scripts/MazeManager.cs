...
public class MazeManager : MonoBehaviour
{
    ...
    [SerializeField] private float _particles=300; 
    //[SerializeField] private int _puryToComplete;
    ...
    private void Solver_OnCollision(ObiSolver s, ObiSolver.ObiCollisionEventArgs e)
    {
        ...
        foreach (Oni.Contact contact in e.contacts)
        {
            ...
            if (contact.distance < 0.01f)
            {
                ...
                {
                    if (colorizer.collider == col)
                    {
                        solver.userData[contact.bodyA] = colorizer.color;
    ...
    {
        var completion = finishedParticles / _particles;
        //int completion = Mathf.CeilToInt(finishedParticles / 700.0f* 100);
        //completionLabel.text = completion + "% Completed";
        //int purity = Mathf.CeilToInt((1 - coloredParticles / 600.0f) * 100);
        //if(completion>=99&&purity<3)_onComplete?.Invoke();
        _progressBar?.SetValue(completion);
        //if(completion>=_waterToComplete)_onComplete?.Invoke();
